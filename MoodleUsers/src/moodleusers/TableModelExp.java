...
class TableModelExp extends DefaultTableModel {
    ...
    public void addColumn() {
        if (getColumnCount() == 0) {
            addColumn("", (Vector) null);
        } else {
            addColumn(columnName[Count++], (Vector) null);
        }
        if (Count == columnName.length) {
            ...
            for (int countColumnNames = 0, countColumnNames0 = 0; countColumnNames < columnName.length; countColumnNames++) {
                if (countColumnNames < Count) {
                    columnName[countColumnNames] = columnNamesCopy[countColumnNames];
                } else {
                    if (!(columnName[countColumnNames - 25].equals(columnNamesCopy[countColumnNames0] + columnNamesCopy[countColumnNames0]))) {
                        columnName[countColumnNames] = columnNamesCopy[countColumnNames0] + columnNamesCopy[countColumnNames0];
                    }
                    countColumnNames0++;
                }
            }
